...
This folder contains the HTTP client layer for communicating with the AWS API Gateway backend (or a mock service for local prototype testing). All network requests originate from this module. It defines the `awsSync.ts` client for uploading attendance records and downloading worksite configurations, plus `types.ts` for all request/response payload shapes.
